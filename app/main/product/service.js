...
class ProductService extends BaseServiceCRUD {
  ...
  async getMany(query) {
    const search_columns = ['producer.country', 'producer.name', 'product.nameProduct']
    let builder = this.model.queryBuilder(query).eager('[categories, rating]').where('isActive', true);
    if (query.keyword) {
      builder = await this.model.query()
        .select(['product.*', 'producer.name', 'producer.country'])
        .innerJoin('producer', 'product.producerId', '=', 'producer.id')
        .where(function (subquery) {
          search_columns.forEach((column) => {
            subquery.orWhere(`${column}`, 'ILIKE', `%${query.keyword.toUpperCase()}%`)
          })
        })
      return {
        results: builder,
        tottal: builder.length
      }
    }
